...
module.exports = cmd => {
  if (!argv) {
    argv = program
      ...
      .option('-r, --registry [registry]', 'registry url, default is ' + config.knpmRegistry)
      .option('-w, --registryweb [registryweb]', 'web url, default is ' + config.knpmHost)
      ...
      .option('--ignore-custom-config', 'ignore custom .knpmrc')
  ...
  argv.on('version', function() {
    console.log('knpm@%s (%s)%snpm@%s (%s)%snode@%s (%s)%snpminstall@%s (%s)%sprefix=%s %s%s %s %s %sregistry=%s',
      pkg.version,
      __filename,
      os.EOL,
  ...
  // console.log(process.argv.slice(), 'process.argv.slice()')
  ...
  argv.registryweb = argv.registryweb || config.knpmHost;
  argv.cache = cacheInfo || config.cache;

  // console.log(argv.args, 'argv.args')
  if (!argv.args.length) {
    help(argv);
  }
